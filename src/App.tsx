...
import { bobs27StorageKey, scoreBobs27, type Bobs27Game } from "./bobs27";

type Page = "practice" | "darts" | "history" | "stats" | "settings";
type PracticeView = "home" | "setup" | "score" | "review" | "match" | "x01" | "practice121" | "bobs27";
type RoutineId = "cricket-mpd" | "jdc-challenge" | "cricket-match" | "x01-match" | "practice-121" | "bobs-27";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    function handlePopState(event: PopStateEvent) {
      ...
      const locallySaved = current.practiceView === "match" || current.practiceView === "x01" || current.practiceView === "practice121" || current.practiceView === "bobs27";
  ...
  function beginPractice(nextRoutine: RoutineId) { setError(""); setRoutine(nextRoutine); setDarts([]); setJdcDarts([]); setSelectedDartSetId(""); setPracticeNotes(""); setStartedAt(null); pushScreen("practice", nextRoutine === "cricket-match" ? "match" : nextRoutine === "x01-match" ? "x01" : nextRoutine === "practice-121" ? "practice121" : nextRoutine === "bobs-27" ? "bobs27" : "setup"); }
  ...
  function leavePractice(nextPage: Page = "practice") {
    ...
    const locallySaved = practiceView === "match" || practiceView === "x01" || practiceView === "practice121" || practiceView === "bobs27";
  ...
  return <div className="app-shell">
    ...
    <main className={practiceView !== "home" ? "content content-score" : "content"}>
      ...
      {page === "practice" && practiceView === "practice121" && <Practice121 userId={user.uid} dartSets={dartSets} onExit={() => leavePractice()} onSaved={async () => { await refreshSessions(user.uid); leaveFlow("practice", 1); }} />}
      {page === "practice" && practiceView === "bobs27" && <Bobs27 userId={user.uid} dartSets={dartSets} onExit={() => leavePractice()} onSaved={async () => { await refreshSessions(user.uid); leaveFlow("practice", 1); }} />}
      ...
      {page === "stats" && <><Stats sessions={sessions} /><Bobs27Stats sessions={sessions} /></>}
...
function PracticeHome({ user, sessions, loading, onStart, onHistory }: { user: User; sessions: StoredPracticeSession[]; loading: boolean; onStart: (routine: RoutineId) => void; onHistory: () => void }) {
  ...
  const bobsClassic = sessions.filter((session) => session.routineId === "bobs-27" && session.mode === "classic" && session.status !== "abandoned").map(asBobs27Score);
  const bobsComplete = sessions.filter((session) => session.routineId === "bobs-27" && session.mode === "complete" && session.status !== "abandoned").map(asBobs27Score);
  const routines = [
    ...
    { name: "Bob’s 27", id: "bobs-27" as const, meta: "63 darts · Every double", description: "Classic elimination or complete-board practice", best: bobsClassic.length || bobsComplete.length ? `C ${bobsClassic.length ? Math.max(...bobsClassic.map((result) => result.finalScore)) : "—"} · CP ${bobsComplete.length ? Math.max(...bobsComplete.map((result) => result.finalScore)) : "—"}` : "—" },
  ];
...
function History({ sessions, dartSets, loading, onUpdateDartSet }: { sessions: StoredPracticeSession[]; dartSets: DartSet[]; loading: boolean; onUpdateDartSet: (session: StoredPracticeSession, dartSet?: DartSet) => Promise<void> }) {
  ...
  if (selected) {
    ...
    const isBobs27 = selected.routineId === "bobs-27";
    const cricket = asCricketDarts(selected);
    const jdc = asJdcDarts(selected);
    const practice121 = asPractice121Score(selected);
    const bobs27 = asBobs27Score(selected);
    return <section><button className="text-button back-button" onClick={() => setSelected(null)}>← All sessions</button><header className="review-heading compact"><div><p className="eyebrow">{isBobs27 ? `Bob’s 27 · ${selected.mode === "complete" ? "Complete Practice" : "Classic"}` : is121 ? "121" : isJdc ? "JDC Challenge" : "Cricket MPR"}</p><h1>{formatDate(selected.completedAt)}</h1>{!is121 && <HistoryDartSetEditor session={selected} dartSets={dartSets} onSave={async (dartSet) => { await onUpdateDartSet(selected, dartSet); setSelected({ ...selected, dartSetId: dartSet?.id, dartSetSnapshot: dartSet ? snapshotDartSet(dartSet) : undefined }); }} />}</div><div className="result-summary"><span>{isBobs27 ? "Final score" : is121 ? "Highest checkout" : isJdc ? "Total score" : "MPR"}</span><strong>{isBobs27 ? bobs27.finalScore : is121 ? practice121.highestCheckoutCompleted ?? "—" : isJdc ? jdcTotalScore(jdc) : formatMpr(marksPerRound(cricket))}</strong><small>{isBobs27 ? `${selected.status === "eliminated" ? "Eliminated" : selected.status === "abandoned" ? "Abandoned" : "Completed"} · ${bobs27.totalHits}/${bobs27.dartsThrown} hits` : is121 ? `${practice121.targetsCompleted} completed · ${practice121.totalDartsThrown} darts` : isJdc ? "57 darts" : `${totalMarks(cricket)} total marks`}</small></div></header>{selected.notes && <section className="saved-notes"><p className="eyebrow">Practice notes</p><p>{selected.notes}</p></section>}{isBobs27 ? <Bobs27History session={selected} /> : is121 ? <Practice121History session={selected} /> : isJdc ? <JdcTable darts={jdc} /> : <VisitTable darts={cricket} />}</section>;
  }
...
function SessionList({ sessions, loading, onSelect }: { sessions: StoredPracticeSession[]; loading: boolean; onSelect?: (session: StoredPracticeSession) => void }) {
  ...
  return <div className="session-list">{sessions.map((session) => { const isJdc = session.routineId === "jdc-challenge"; const is121 = session.routineId === "practice-121"; const isBobs27 = session.routineId === "bobs-27"; const practice121 = asPractice121Score(session); const bobs27 = asBobs27Score(session); return <button key={session.id} onClick={() => onSelect?.(session)} disabled={!onSelect} className="session-card"><span><strong>{isBobs27 ? `Bob’s 27 · ${session.mode === "complete" ? "Complete" : "Classic"}` : is121 ? "121" : isJdc ? "JDC Challenge" : "Cricket MPR"}</strong><small>{formatDate(session.completedAt)}{session.dartSetSnapshot ? ` · ${session.dartSetSnapshot.name}` : ""}</small></span><span><strong>{isBobs27 ? bobs27.finalScore : is121 ? practice121.highestCheckoutCompleted ?? "—" : isJdc ? jdcTotalScore(asJdcDarts(session)) : formatMpr(marksPerRound(asCricketDarts(session)))}</strong><small>{isBobs27 ? session.status === "abandoned" ? "Abandoned" : "Final score" : is121 ? "Highest checkout" : isJdc ? "Score" : "MPR"}</small></span><b aria-hidden="true">{onSelect ? "→" : ""}</b></button>; })}</div>;
}
...
function Bobs27History({ session }: { session: StoredPracticeSession }) {
  return <div className="visit-table"><div className="visit-row bobs27-history-row visit-head"><span>Double</span><span>Hits</span><span>Score</span></div>{(session.rounds || []).map((round) => <div className="visit-row bobs27-history-row" key={round.target}><strong>{round.label}</strong><span>{round.hits} / 3</span><strong>{round.scoreAfter}</strong></div>)}</div>;
}
...
function asBobs27Score(session: StoredPracticeSession) { return scoreBobs27({ version: 1, mode: session.mode || "classic", startedAt: new Date(0).toISOString(), rounds: session.rounds || [], status: session.status } satisfies Bobs27Game); }
...
function hasStoredMatch(view: PracticeView, userId: string) {
  try {
    ...
    if (view === "bobs27") return Boolean(localStorage.getItem(bobs27StorageKey(userId)));
    return false;
  } catch { return false; }
}

function Bobs27Stats({ sessions }: { sessions: StoredPracticeSession[] }) {
  const saved = sessions.filter((session) => session.routineId === "bobs-27");
  if (!saved.length) return null;
  const eligible = saved.filter((session) => session.status !== "abandoned");
  const classic = eligible.filter((session) => session.mode === "classic");
  const complete = eligible.filter((session) => session.mode === "complete");
  const allRounds = saved.flatMap((session) => session.rounds || []);
  const totalHits = allRounds.reduce((sum, round) => sum + round.hits, 0);
  const accuracy = allRounds.length ? (totalHits / (allRounds.length * 3)) * 100 : 0;
  return <section className="routine-stats bobs27-lifetime"><h2>Bob’s 27</h2><div className="stats-grid"><article><span>Classic best</span><strong>{classic.length ? Math.max(...classic.map((session) => asBobs27Score(session).finalScore)) : "—"}</strong></article><article><span>Complete Practice best</span><strong>{complete.length ? Math.max(...complete.map((session) => asBobs27Score(session).finalScore)) : "—"}</strong></article><article><span>Overall accuracy</span><strong>{accuracy.toFixed(1)}%</strong></article></div><section className="target-stats"><h3>Accuracy by double</h3>{Array.from(new Set(allRounds.map((round) => round.target))).map((target) => { const rounds = allRounds.filter((round) => round.target === target); const hits = rounds.reduce((sum, round) => sum + round.hits, 0); const rate = hits / (rounds.length * 3); return <div key={target}><strong>{rounds[0]?.label}</strong><span><i style={{ width: `${rate * 100}%` }} /></span><b>{(rate * 100).toFixed(0)}%</b></div>; })}</section><p className="bobs27-stat-note">{classic.filter((session) => session.status === "eliminated").length} Classic eliminations · {saved.filter((session) => session.status === "abandoned").length} saved abandoned sessions</p></section>;
}
