...
export function Practice121({ userId, onExit, onSaved }: { userId: string; onExit: () => void; onSaved: () => Promise<void> }) {
  ...
  if (!game) return <Practice121Setup onCancel={onExit} onStart={setGame} />;
  return <Practice121Board game={game} setGame={setGame} userId={userId} onExit={onExit} onSaved={onSaved} />;
}

function Practice121Setup({ onCancel, onStart }: { onCancel: () => void; onStart: (game: Practice121Game) => void }) {
  ...
  return <section className="practice-setup practice121-setup"><button className="text-button back-button" onClick={onCancel}>← Practice</button><div className="setup-card"><p className="eyebrow">Checkout practice</p><h1>121</h1><p>Check out each target in nine darts. Fast finishes earn a life; failed attempts cost one.</p><fieldset className="x01-choice"><legend>Starting lives</legend>{[1, 3, 5].map((value) => <label className={parsed === value ? "selected" : ""} key={value}><input type="radio" checked={parsed === value} onChange={() => setLives(String(value))} />{value}</label>)}</fieldset><label>Custom lives <small>1–99</small><input type="number" inputMode="numeric" min="1" max="99" step="1" value={lives} onChange={(event) => setLives(event.target.value)} /></label>{!valid && <p className="notice" role="alert">Choose a whole number from 1 to 99.</p>}<div className="review-actions"><button className="button button-secondary" onClick={onCancel}>Cancel</button><button className="button" disabled={!valid} onClick={start}>Start game</button></div></div></section>;
}

function Practice121Board({ game, setGame, userId, onExit, onSaved }: { game: Practice121Game; setGame: (game: Practice121Game) => void; userId: string; onExit: () => void; onSaved: () => Promise<void> }) {
