...
export function X01CalculatorBoard({ game, setGame, onExit, onNewGame }: { game: X01TotalGame; setGame: (game: StoredX01Game) => void; onExit: () => void; onNewGame: () => void }) {
  ...
  return <section className="cricket-match-shell x01-shell">
    ...
    {!winner && <section className="x01-calculator" aria-label="Turn score calculator">
      ...
      <div className="x01-keypad">{[1, 2, 3, 4, 5, 6, 7, 8, 9].map((digit) => <button type="button" onClick={() => setGame(appendX01Digit(game, digit))} key={digit}>{digit}</button>)}<button type="button" className="key-function" onClick={() => setGame(setX01Draft(game, ""))}>Clear</button><button type="button" onClick={() => setGame(appendX01Digit(game, 0))}>0</button><button type="button" className="key-function" aria-label="Backspace" onClick={() => setGame(setX01Draft(game, game.draft.slice(0, -1)))}>⌫</button></div>
